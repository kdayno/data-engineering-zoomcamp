...
resource "google_cloud_run_service" "run_service" {
  ...
  template {
    spec {
      service_account_name = "mage-terraform-runner@data-eng-zoomcamp-2024-412913.iam.gserviceaccount.com"
      containers {
        ...
        env {
          name  = "mage-tf-runner-keyfile"
          value = "/secrets/Mage-Terraform-Runner"
        }
        volume_mounts {
          name       = "secrets-mage"
          mount_path = "/secrets"
      ...
      volumes {
        name = "secrets-mage"
        secret {
          secret_name = "Mage-Terraform-Runner"
          items {
            key  = "latest"
            path = "Mage-Terraform-Runner"
          }
        }
      }
    }
